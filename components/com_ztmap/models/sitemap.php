 ...
 * @since          2.0
 */
class ZtmapModelSitemap extends JModelItem
{
     ...
     * @var        string
     */
    protected $_context = 'com_ztmap.sitemap';
    ...
    public function &getItem($pk = null)
    {
        ...
        if (!$pk)
        {
            ...
            $query->select('id')->from('#__ztmap_sitemap')->where('is_default=1');
            $db->setQuery($query);
            $pk = $db->loadResult();
        }
        ...
        if (!isset($this->_item[$pk]))
        {
            try
            {
                ...
                $query->from('#__ztmap_sitemap AS a');
    ...
    public function getItems()
    {
        ...
        {
            return ZtmapHelper::getMenuItems($item->selections);
        }
        return false;
    }
    ...
    {
        return ZtmapHelper::getExtensions();
    }
    ...
    public function getSitemapItems($view = null)
    {
        ...
        $query = "select * from #__ztmap_items where view='$view' and sitemap_id=" . $pk;
        $db->setQuery($query);
    ...
    function chageItemPropery($uid, $itemid, $view, $property, $value)
    {
        ...
        if (!$isNew)
        {
            $query = 'UPDATE #__ztmap_items SET properties=\'' . $db->escape($properties) . "' where uid='" . $db->escape($uid) . "' and itemid=$itemid and view='$view' and sitemap_id=" . $pk;
        } else
        {
            $query = 'INSERT #__ztmap_items (uid,itemid,view,sitemap_id,properties) values ( \'' . $db->escape($uid) . "',$itemid,'$view',$pk,'" . $db->escape($properties) . "')";
        }
        $db->setQuery($query);
    ...
    function toggleItem($uid, $itemid)
    {
        ...
        $displayer = new ZtmapDisplayer($app->getParams(), $sitemap);
        ...
        $db = JFactory::getDBO();
        $query = "UPDATE #__ztmap_sitemap set excluded_items='" . $db->escape($str) . "' where id=" . $sitemap->id;
        $db->setQuery($query);
        $db->query();
        return $state;
    }
