...
class ztmap_com_content
{
        ...
        switch ($view)
        {
            ...
            case 'article':
                ...
                if (($row = $db->loadObject()) != NULL)
                {
                    ...
                    {
                        $node->images = ZtmapHelper::getImages($text, JArrayHelper::getValue($params, 'max_images', 1000));
                    }

                    if ($params['add_pagebreaks'])
                    {
                        $node->subnodes = ZtmapHelper::getPagebreaks($text, $node->link);
     ...
     * @since  1.0
     */
    static function getTree($ztmap, $parent, &$params)
    {
        $db = JFactory::getDBO();
        ...
        $expand_categories = ( $expand_categories == 1 || ( $expand_categories == 2 && $ztmap->view == 'xml') || ( $expand_categories == 3 && $ztmap->view == 'html') || $ztmap->view == 'navigator');
        ...
        $expand_featured = ( $expand_featured == 1 || ( $expand_featured == 2 && $ztmap->view == 'xml') || ( $expand_featured == 3 && $ztmap->view == 'html') || $ztmap->view == 'navigator');
        $params['expand_featured'] = $expand_featured;

        //----- Set expand_featured param
        $include_archived = JArrayHelper::getValue($params, 'include_archived', 2);
        $include_archived = ( $include_archived == 1 || ( $include_archived == 2 && $ztmap->view == 'xml') || ( $include_archived == 3 && $ztmap->view == 'html') || $ztmap->view == 'navigator');
        $params['include_archived'] = $include_archived;

        //----- Set show_unauth param
        $show_unauth = JArrayHelper::getValue($params, 'show_unauth', 1);
        $show_unauth = ( $show_unauth == 1 || ( $show_unauth == 2 && $ztmap->view == 'xml') || ( $show_unauth == 3 && $ztmap->view == 'html'));
        $params['show_unauth'] = $show_unauth;

        //----- Set add_images param
        $add_images = JArrayHelper::getValue($params, 'add_images', 0) && $ztmap->isImages;
        $add_images = ( $add_images && $ztmap->view == 'xml');
        $params['add_images'] = $add_images;
        $params['max_images'] = JArrayHelper::getValue($params, 'max_images', 1000);

        //----- Set add pagebreaks param
        $add_pagebreaks = JArrayHelper::getValue($params, 'add_pagebreaks', 1);
        $add_pagebreaks = ( $add_pagebreaks == 1 || ( $add_pagebreaks == 2 && $ztmap->view == 'xml') || ( $add_pagebreaks == 3 && $ztmap->view == 'html') || $ztmap->view == 'navigator');
        ...
        switch ($view)
        {
            case 'category':
                ...
                {
                    $result = self::expandCategory($ztmap, $parent, $id, $params, $parent->id);
                }
                break;
            case 'featured':
                if ($params['expand_featured'])
                {
                    $result = self::includeCategoryContent($ztmap, $parent, 'featured', $params, $parent->id);
                }
                break;
            case 'categories':
                if ($params['expand_categories'])
                {
                    $result = self::expandCategory($ztmap, $parent, ($id ? $id : 1), $params, $parent->id);
                }
                break;
            case 'archive':
                if ($params['expand_featured'])
                {
                    $result = self::includeCategoryContent($ztmap, $parent, 'archived', $params, $parent->id);
                }
                break;
            case 'article':
                ...
                if ($params['add_pagebreaks'])
                {
                    ...
                    self::printNodes($ztmap, $parent, $params, $subnodes);
                }
        }
        return $result;
    }
     ...
     */
    static function expandCategory($ztmap, $parent, $catid, &$params, $itemid)
    {
        $db = JFactory::getDBO();
        ...
        if (count($items) > 0)
        {
            ...
            foreach ($items as $item)
            {
                ...
                if ($ztmap->printNode($node))
                {
                    self::expandCategory($ztmap, $parent, $item->id, $params, $node->itemid);
                }
            }
            $ztmap->changeLevel(-1);
        }
        ...
        self::includeCategoryContent($ztmap, $parent, $catid, $params, $itemid);
        return true;
    }
     ...
     * @since 2.0
     */
    static function includeCategoryContent($ztmap, $parent, $catid, &$params, $Itemid)
    {
        $db = JFactory::getDBO();
        ...
        if ($params['max_art_age'] || $ztmap->isNews)
        {
            $days = (($ztmap->isNews && ($params['max_art_age'] > 3 || !$params['max_art_age'])) ? 3 : $params['max_art_age']);
            $where[] = "( a.created >= '"
        ...
        if (count($items) > 0)
        {
            ...
            foreach ($items as $item)
            {
                ...
                if ($ztmap->printNode($node) && $node->expandible)
                {
                    self::printNodes($ztmap, $parent, $params, $subnodes);
                }
            }
    ...
    static private function printNodes($ztmap, $parent, &$params, &$subnodes)
    {
        $ztmap->changeLevel(1);
        $i = 0;
