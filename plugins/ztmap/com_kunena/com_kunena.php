 ...
 * @license     GPL v2 
 * 
 * @author Guillermo Vargas, http://www.jooztmap.com
...
class ztmap_com_kunena
{
    ...
    {
        if ($ztmap->isNews) // This component does not provide news content. don't waste time/resources
            return false;

        // Make sure that we can load the kunena api
        if (!ztmap_com_kunena::loadKunenaApi())
        {
            return false;
        }
        ...
        $include_topics = ( $include_topics == 1 || ( $include_topics == 2 && $ztmap->view == 'xml') || ( $include_topics == 3 && $ztmap->view == 'html') || $ztmap->view == 'navigator');
        ...
        if ($include_topics)
        {
            ...
            $params['include_pagination'] = ($ztmap->view == 'xml');
            ...
            $params['days'] = false;
            if (intval($days))
                $params['days'] = ($ztmap->now - (intval($days) * 86400));
        }

        $params['table_prefix'] = ztmap_com_kunena::getTablePrefix();

        ztmap_com_kunena::getCategoryTree($ztmap, $parent, $params, $catid);
    }
    ...
    static function getCategoryTree($ztmap, $parent, &$params, $parentCat)
    {
        $db = JFactory::getDBO();
        ...
        foreach ($categories as $cat)
        {
            ...
            if ($ztmap->printNode($node) !== FALSE)
            {
                ztmap_com_kunena::getCategoryTree($ztmap, $parent, $params, $cat->id);
            }
        }
