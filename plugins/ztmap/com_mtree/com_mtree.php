...
class ztmap_com_mtree
{
    ...
    static function getTree($ztmap, $parent, &$params)
    {
        ...
        $include_links = ( $include_links == 1 || ( $include_links == 2 && $ztmap->view == 'xml') || ( $include_links == 3 && $ztmap->view == 'html') || $ztmap->view == 'navigator');
        ...
        if ($include_links)
        {
            ...
            if (intval($days))
                $params['days'] = ' AND a.link_created >=\'' . date('Y-m-d H:i:s', ($ztmap->now - ($days * 86400))) . "' ";
        }

        ztmap_com_mtree::getMtreeCategory($ztmap, $parent, $params, $catid);
    }

    /* Returns URLs of all Categories and links in of one category using recursion */

    static function getMtreeCategory($ztmap, $parent, &$params, $catid)
    {
        ...
        foreach ($rows as $row)
        {
            ...
            {
                ztmap_com_mtree::getMtreeCategory($ztmap, $parent, $params, $row->cat_id);
            }
        }
